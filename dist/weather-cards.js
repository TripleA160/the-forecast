...
export class CurrentWeatherCard {
    ...
    pressure = 0;
    visibility = 0;
    constructor(date = new Date(), element = null) {
        this.date = date;
    ...
    #initializeElement() {
        ...
        precipitationElement.append(precipitationIcon, precipitationLabel, precipitationText);
    ...
    updateElement() {
        this.element.querySelector(".current-weather-temp-value").textContent =
            this.temp.toString() + "°";
        this.element.querySelector(".current-weather-apparent-temp-value").textContent = this.apparentTemp.toString() + "°";
        this.element.querySelector(".current-weather-description").textContent =
            this.description;
        this.element.querySelector(".current-weather-precipitation-value").textContent = this.precipitation.toString() + "%";
        this.element.querySelector(".current-weather-humidity-value").textContent = this.humidity.toString() + "%";
        this.element.querySelector(".current-weather-uv-value").textContent =
            this.uv.toString() + "/11";
        this.element.querySelector(".current-weather-wind-direction").style.rotate = this.windDirection + "deg";
        ...
        this.element.querySelector(".current-weather-wind-unit").textContent =
            "km/h";
        this.element.querySelector(".current-weather-pressure-value").textContent = this.pressure.toString();
        this.element.querySelector(".current-weather-pressure-unit").textContent =
            "mb";
        this.element.querySelector(".current-weather-visibility-value").textContent = this.visibility.toString();
        this.element.querySelector(".current-weather-visibility-unit").textContent = "km";
...
export class HourlyWeatherCard {
    ...
    async #initializeElement() {
        ...
        this.element.append(label, this.daysNav.element, this.hoursNav.navContainer);
        ...
        await this.daysNav.selectDay(this.daysNav.days.find((day) => day.description === "Today"), true);
