import { fetchWeatherApi, fetchCities, fetchUserLocation, } from "./api-fetcher.js";
import { CurrentWeatherCard, HourlyWeatherCard } from "./weather-cards.js";
// Initial state
let locationData;
export let weatherData;
let searchResults;
let preferedSearch = "false", grantedLocationPermission = false, canGetLocation = false;
export let singleDayView = false;
let headerElement = document.querySelector("header"), headerWeatherElement = headerElement.querySelector(".header-weather"), searchBar = headerElement.querySelector(".search-bar"), searchInput = searchBar.querySelector(".search-input"), searchResultsElement = searchBar.querySelector(".search-results"), contentElement = document.querySelector(".content");
export let locationMessage = document.querySelector(".location-message");
export let currentWeatherCard, hourlyWeatherCard;
window.addEventListener("load", async () => {
    ...
    searchInput.addEventListener("blur", (e) => {
        if (!searchResultsElement.contains(e.relatedTarget)) {
            searchResultsElement.innerHTML = "";
            searchResultsElement.classList.remove("expanded");
        }
    });
...
async function updateUserWeather() {
    ...
    await updateWeatherData();
    if (!currentWeatherCard) {
        currentWeatherCard = new CurrentWeatherCard(new Date(weatherData.current.time));
...
async function updateWeatherData() {
    const weatherResponse = await fetchWeatherApi(locationData.latitude, locationData.longitude, locationData.timezone);
    weatherData = {
        current: {
...
async function updateSearchResults() {
    ...
    for (let i = 0; i < searchResults.length; i++) {
        ...
        const name = searchResults[i].name, admin = searchResults[i].admin1, country = searchResults[i].country;
        ...
        resultItem.addEventListener("click", async () => {
            if (!(grantedLocationPermission && canGetLocation) &&
                preferedSearch != "true") {
                preferedSearch = "true";
