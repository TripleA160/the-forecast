...
export async function fetchWeatherApi(latitude, longitude, timezone) {
    try {
...
export async function fetchCities(query) {
    try {
        const response = isRTL(query)
            ? await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${query}&count=10&language=ar&format=json`)
            : await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${query}&count=10&language=en&format=json`);
